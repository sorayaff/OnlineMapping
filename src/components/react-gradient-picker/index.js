...
class ReactGradientColorPicker extends React.Component {
    ...
    _refreshCanvas() {
        ...
        this.canvas.each(function renderCanvas() {
          ...
          else if(type1=="descrete")
          {
            for (var i = 0, j = -1, k = 0, c, a; i < rootWidth; ++i) {
              if(offsets[k]*rootWidth<i)
              {
                k = k+1;
              }
              c = d3.rgb(colors[k-1]);
              // console.log(rootWidth,i,offsets);
              a = _localOpacityScale(i * 1.0 / rootWidth);
              image.data[++j] = c.r;
              image.data[++j] = c.g;
              image.data[++j] = c.b;
        ...
        var drag = d3.drag()
            ...
            .on('end', this._dragHandlerEnd.bind(this));
...
export default ReactGradientColorPicker
